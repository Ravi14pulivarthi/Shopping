...
function Kids() {
    ...
    useEffect(()=>{
     ...
     then( (result)=>   setdata(result.categories[2].category_products))
    },[])
...
data.map((values)=>{
 ...
 <div  style={{ height:"18rem",width:"15rem", backgroundImage:`url(${values.image})`,backgroundRepeat:"no-repeat",backgroundSize:"cover",borderRadius:"10px"}} >
   <p style={{border:"0px solid",width:"150px",height:"30px",backgroundColor:"white",color:"black",textAlign:"center",position:"relative",left:"20px",top:"20px"}} >{values.badge_text}</p> </div>
